...
void transform_pointcloud_with_normals_and_unit_vectors(const pcl::PointCloud<PointT>& cloud_in,
    ...
    if (cloud_in.is_dense) {
        for (std::size_t i = 0; i < cloud_out.points.size(); ++i) {
            ...
            tf.so3(cloud_in[i].data_u, cloud_out[i].data_u);
        }
    }
...
void transform_pointcloud_with_normals_and_unit_vectors(const pcl::PointCloud<PointT>& cloud_in,
    ...
    transform_pointcloud_with_normals_and_unit_vectors(cloud_in, cloud_out, transform.template cast<float>(),
            copy_all_fields);
}
...
void transform_pointcloud_with_normals_and_unit_vectors(const pcl::PointCloud<PointT>& cloud_in,
    ...
    transform_pointcloud_with_normals_and_unit_vectors(cloud_in, cloud_out, transform.matrix(), copy_all_fields);
}
